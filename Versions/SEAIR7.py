...
def open_edit_window():
    ...
    edit_window.title("ویرایش شرایط اولیه")
    
    entries = {}
    for idx, (ic, value) in enumerate(initial_conditions.items()):
        label = ttk.Label(edit_window, text=ic)
    ...
    def save_changes():
        for ic, entry in entries.items():
            initial_conditions[ic] = float(entry.get())
        
        # Calculate the new total population
        new_total_population = sum(initial_conditions.values())
        
        # Check if the new total population matches the displayed total population
        if new_total_population != N:
            messagebox.showerror("خطا", "جمعیت کل با مجموع مقادیر شرایط اولیه برابر نیست و باید تصحیح گردد.")
            return  # Exit the function if there's an error
        
        # If valid, update the total population entry and proceed
        N = new_total_population
        total_population_entry.delete(0, tk.END)
        total_population_entry.insert(0, str(N))
        edit_window.destroy()
        update_model()

    
    save_button = ttk.Button(edit_window, text="ذخیره تغییرات", command=save_changes)
    save_button.grid(row=len(initial_conditions), column=0, columnspan=2, pady=10)

def toggle_initial_conditions_edit():
    global edit_mode
    edit_mode = not edit_mode
    state = 'normal' if edit_mode else 'readonly'
    for ic, entry in ic_entries.items():
        entry.config(state=state)
        if not edit_mode:
            # Update the initial_conditions dictionary with the current values
            initial_conditions[ic] = float(entry.get())
    edit_button.config(text="ذخیره شرایط اولیه" if edit_mode else "ویرایش شرایط اولیه")
    if not edit_mode:
        update_model()
...
# Define lists for splitting the parameters
...
days_slider_label.grid(row=len(first_column_params), column=0, padx=10, pady=5)

days_slider = tk.Scale(param_frame, from_=10, to=365, orient=tk.HORIZONTAL)
days_slider.set(80)  # مقدار پیش‌فرض
days_slider.grid(row=len(first_column_params), column=1, padx=10, pady=5)

# Quarantine Slider
quarantine_slider_label = ttk.Label(param_frame, text="درصد قرنطینه:")
quarantine_slider_label.grid(row=len(first_column_params) + 1, column=0, padx=10, pady=5)

quarantine_slider = tk.Scale(param_frame, from_=0, to=100, orient=tk.HORIZONTAL)
quarantine_slider.set(0)  # مقدار پیش‌فرض
quarantine_slider.grid(row=len(first_column_params) + 1, column=1, padx=10, pady=5)

# Total Population Display
total_population_label = ttk.Label(param_frame, text="جمعیت کل:")
total_population_label.grid(row=len(first_column_params) + 2, column=0, padx=10, pady=5)

total_population_entry = ttk.Entry(param_frame, state='normal')
total_population_entry.grid(row=len(first_column_params) + 2, column=1, padx=10, pady=5)
total_population_entry.insert(0, str(N))

# Edit Initial Conditions Button
edit_button = ttk.Button(param_frame, text="ویرایش شرایط اولیه", command=open_edit_window)
edit_button.grid(row=len(first_column_params) + 3, column=0, columnspan=2, pady=10)
