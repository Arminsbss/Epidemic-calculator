...
def update_model():
    ...
    for param in parameters:
        if param not in ["hospitalization_duration","D_mortality","D_recovery_mild","vaccination_day","birth_rate",    
...
def seair_model(S, E, A, I, Ri, Ra, Is, D, Va, Im, t, RZero, sigma, natural_mortality_rate):
    ...
    betaI = RZero / 2.9  # Transmission rate for symptomatic cases
    betaA = (RZero * 0.35) / 2.9  # Transmission rate for asymptomatic cases
    aE = sigma * 0.4  # Rate of progression from exposed to asymptomatic
    iE = sigma * 0.6  # Rate of progression from exposed to symptomatic
    gammaI = 1/2.9  # 1/time of illness for symptomatic individuals
    gammaA = 1/2.9  # 1/time of illness for asymptomatic individuals
    p_mild = 14 - 2.9  # Recovery time for mild illness (adjust as needed)
    p_severe = 0.2  # Fraction of cases that become severe
    p_fatal = 0  # Mortality rate
    birth_rate = 0  # Example birth rate, adjust as needed
    
    # Differential equations (including natural mortality)
    dSdt = -betaI * I * S - betaA * A * S + birth_rate * S - natural_mortality_rate * S
    dEdt = betaI * I * S + betaA * A * S - aE * E - natural_mortality_rate * E
    dAdt = aE * E - gammaA * A - natural_mortality_rate * A
    dIdt = iE * E - gammaI * I - natural_mortality_rate * I
    dRidt = gammaI * I - natural_mortality_rate * Ri
    dRadt = gammaA * A - natural_mortality_rate * Ra
    dIsdt = p_severe * I - p_fatal * I - natural_mortality_rate * Is  # Severe symptomatic progression
    dDdt = p_fatal * I  # Deaths due to illness
    dVadt = birth_rate * S  # Births to the vaccinated
    dImdt = 0  # No explicit equation for immunized (adjust if needed)
...
def euler_method(S0, E0, A0, I0, Ri0, Ra0, Is0, D0, Va0, Im0, t, dt, RZero, sigma, natural_mortality_rate=0.0001):
    S, E, A, I, Ri, Ra, Is, D, Va, Im = [S0], [E0], [A0], [I0], [Ri0], [Ra0], [Is0], [D0], [Va0], [Im0]
    
    # Loop for each time step in the simulation
    for i in range(1, len(t)):
        dSdt, dEdt, dAdt, dIdt, dRidt, dRadt, dIsdt, dDdt, dVadt, dImdt = seair_model(
            S[-1], E[-1], A[-1], I[-1], Ri[-1], Ra[-1], Is[-1], D[-1], Va[-1], Im[-1], t[i], RZero, sigma, natural_mortality_rate
        )

        # Euler integration for each compartment, ensuring no negative values
        S_new = max(S[-1] + dSdt * dt, 0)
        E_new = max(E[-1] + dEdt * dt, 0)
        A_new = max(A[-1] + dAdt * dt, 0)
        I_new = max(I[-1] + dIdt * dt, 0)
        Ri_new = max(Ri[-1] + dRidt * dt, 0)
        Ra_new = max(Ra[-1] + dRadt * dt, 0)
        Is_new = max(Is[-1] + dIsdt * dt, 0)
        D_new = max(D[-1] + dDdt * dt, 0)
        Va_new = max(Va[-1] + dVadt * dt, 0)
        Im_new = max(Im[-1] + dImdt * dt, 0)
        
        # Check if the total population exceeds N and scale it back if necessary
...
def run_and_plot_model():
    ...
    RZero = 2.5
    sigma = 5.2 / 3.6
    natural_mortality_rate = 0.0001  # Set a default value for natural mortality rate

    S, E, A, I, Ri, Ra, Is, D, Va, Im = euler_method(S0, E0, A0, I0, Ri0, Ra0, Is0, D0, Va0, Im0, t, dt, RZero, sigma, natural_mortality_rate)
