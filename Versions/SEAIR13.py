...
def update_model():
    ...
    # Update all parameters from user inputs in the Tkinter entries
    for param in parameters:
        if param in ic_entries:
    ...
    run_and_plot_model()  # Ensure this is called to update the plot
...
def calculate_R0():
    ...
    # Calculate R0
    R0 = (betta * (1 + epsilon * omega)) / (alpha + gamma)
...
def update_R0_value():
    ...
    if advanced_r0_var.get() == 0:  # If advanced settings are not checked
        ...
        default_R0 = (betta * (1 + epsilon * omega)) / (alpha + gamma)
...
def euler_method(S0, E0, A0, I0, Ri0, Ra0, Is0, D0, Va0, Im0, t, dt, RZero, sigma, natural_mortality_rate=0):
    ...
    for i in range(1, len(t)):
        ...
        S_new = max(S[-1] + ds * dt, 0)
        E_new = max(E[-1] + de * dt, 0)
        A_new = max(A[-1] + da * dt, 0)
        I_new = max(I[-1] + di * dt, 0)
        Ri_new = max(Ri[-1] + dRi * dt, 0)
        Ra_new = max(Ra[-1] + dRa * dt, 0)
        Is_new = max(Is[-1] + dIs * dt, 0)
        D_new = max(D[-1] + dD * dt, 0)
        Va_new = max(Va[-1] + dVa * dt, 0)
        Im_new = max(Im[-1] + dIm * dt, 0)
...
days_slider.set(280)  # مقدار پیش‌فرض
...
entry.bind("<Return>", lambda event: update_model())
